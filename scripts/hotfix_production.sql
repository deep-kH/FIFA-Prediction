-- ==============================================================================
-- HOTFIX FOR PRODUCTION DATABASE
-- Run this script in your Supabase SQL Editor to fix the "UPDATE requires WHERE clause" error
-- and to clean up old chat messages automatically.
-- ==============================================================================

-- 1. Fix recalculate_leaderboard to be 100% pg_safeupdate compliant
...
BEGIN
  ...
  -- Use IN clause to satisfy pg_safeupdate for a full table update
  WHERE p.id IN (SELECT id FROM public.profiles);
END;
$$;


-- 2. Fix settle_match to be pg_safeupdate compliant by replacing EXISTS with IN
...
BEGIN
  ...
  -- Safeupdate compliant streak updates
  UPDATE public.profiles p
...
-- 3. Trigger function to delete ALL events when a match is completed
...
-- Make sure the trigger is attached to the matches table
