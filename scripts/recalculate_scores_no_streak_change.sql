-- Recalculate all scores for completed matches without altering streaks
DO $$
DECLARE
...
BEGIN
  ...
  FOR v_match IN SELECT * FROM public.matches WHERE is_completed = true ORDER BY kickoff_time ASC LOOP
    ...
    FOR v_ballot IN SELECT * FROM public.ballots WHERE match_id = v_match.id LOOP
      v_score_pts  := 0;
      v_team_pts   := 0;
      v_scorer_pts := 0;
      v_mcq_pts    := 0;
      ...
      WHERE pa.poll_id = cp.id AND cp.match_id = v_match.id AND pa.user_id = v_ballot.user_id;
      ...
      WHERE cp.match_id = v_match.id AND pa.user_id = v_ballot.user_id;
      ...
      -- Update the ballot without modifying streaks
