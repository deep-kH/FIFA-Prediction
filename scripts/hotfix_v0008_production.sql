-- ==============================================================================
-- HOTFIX FOR PRODUCTION DATABASE (v0008)
-- Run this script in your Supabase SQL Editor.
-- This fixes the "UPDATE requires WHERE clause" error for your CURRENT production database,
-- without requiring the unreleased UI code or missing tables.
-- ==============================================================================
...
BEGIN
  ...
  FOR v_ballot IN SELECT * FROM public.ballots WHERE match_id = p_match_id LOOP
    ...
    ELSIF v_ballot.played_card = 'SAFETY_NET' THEN
      IF v_total_ballot_pts < 2.50 THEN
        -- Haram Ball Override: Guaranteed 2.50 points
  ...
  -- 5. Recalculate Streaks (NEW LOGIC - pg_safeupdate compliant)
  -- Increment streak if accuracy >= 25%
  ...
  -- Reset streak if accuracy < 25% AND they didn't play a SAFETY_NET
  UPDATE public.profiles p
  ...
  -- 6. Recalculate Total Points (pg_safeupdate compliant)
  UPDATE public.profiles p
  SET total_points = (
    SELECT coalesce(sum(b.points_earned), 0) FROM public.ballots b WHERE b.user_id = p.id
  )
  WHERE id IN (
    SELECT user_id FROM public.ballots
  );
