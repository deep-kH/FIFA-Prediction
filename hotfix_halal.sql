-- =============================================
-- Hotfix: Gamification Wildcard logic for Halal Ball
-- =============================================

-- 1. Replace the settle_match function to use the 60% / 40% rule
...
-- 2. Recalculate points for all completed matches using the updated function
DO $$
DECLARE
  m record;
BEGIN
  -- Loop through all completed matches and re-settle them
  FOR m IN SELECT id FROM public.matches WHERE is_completed = true ORDER BY kickoff_time ASC LOOP
    PERFORM public.settle_match(m.id);
  END LOOP;
END;
$$;
