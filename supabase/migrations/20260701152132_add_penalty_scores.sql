...
-- Add penalty scores to ballots
ALTER TABLE public.ballots
ADD COLUMN IF NOT EXISTS predicted_home_penalty_score integer,
ADD COLUMN IF NOT EXISTS predicted_away_penalty_score integer;
  ...
  FOR v_ballot IN SELECT * FROM public.ballots WHERE match_id = p_match_id LOOP
    ...
    IF v_ballot.predicted_home_score IS NOT NULL AND v_ballot.predicted_away_score IS NOT NULL THEN
      ...
      IF v_ballot.predicted_home_score = v_match.home_score AND v_ballot.predicted_away_score = v_match.away_score THEN
        -- If it was a penalty shootout, they ONLY get the 5 pts if they correctly predicted the winner (user's request)
      ...
      -- Check Exact Penalty Match
      IF v_is_penalty_actual AND v_outcome_pred = v_outcome_actual THEN
