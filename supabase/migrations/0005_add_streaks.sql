...
begin
  ...
  -- Increment streak if the user earned ANY points for this match
  update public.profiles p
  set current_streak = current_streak + 1
  where exists (
    select 1 from public.ballots b
    where b.user_id = p.id and b.match_id = p_match_id and b.points_earned > 0
  );

  -- Reset streak to 0 if the user earned 0 points for this match OR skipped the match
  update public.profiles p
  ...
  -- 6. Recalculate all affected users' total_points atomically
  update public.profiles p
  set total_points = (
